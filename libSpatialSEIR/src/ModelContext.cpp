...
namespace SpatialSEIR
{
    ...
    void ModelContext::generateCompartmentsFromPriors()
    {   
        // This is going to be wildly inefficient, but as it's used 
        // once I'm not worried about it. Readability and correctness
        // are far more important than performance here. 


        int nrow = (*(S_star -> nrow));
        int ncol = (*(S_star -> ncol));
        int compartment = (config -> dataModelCompartment);
        int compartmentSize = nrow*ncol;
        memset((S_star -> data), 0,compartmentSize*sizeof(int));
        memset((E_star -> data), 0,compartmentSize*sizeof(int));
        if (compartment == 0)
        {
            memcpy((I_star -> data), Y, compartmentSize*sizeof(int));
            memset((R_star -> data), 0,compartmentSize*sizeof(int));
        }
        else
        {
            memcpy((R_star -> data), Y, compartmentSize*sizeof(int));
            memset((I_star -> data), 0,compartmentSize*sizeof(int));
        }
        ...
        for (i = 0; i < nrow; i++)
        {
            ...
            {
                compIdx = j*nrow + i; 
                (S_star -> data)[compIdx] = (random -> binom((R -> data)[compIdx], p_rs[i]));
                (E_star -> data)[compIdx] = (random -> binom((S -> data)[compIdx], p_se[compIdx]));
                if (compartment != 0)
                {
                    (I_star -> data)[compIdx] = (random -> binom((E -> data)[compIdx], p_ei[i]));
                    // If needed, depart from data and let data model take care of it.
                    if ((R_star -> data)[compIdx] > (I -> data)[compIdx])
                    {
                        (R_star -> data)[compIdx] = (I -> data)[compIdx];
                        modified++;
                    }
                }
                else
                {
                    (R_star -> data)[compIdx] = (random -> binom((I -> data)[compIdx], p_ir[i]));
                    if ((I_star -> data)[compIdx] > (E -> data)[compIdx])
                    {
        ...
        if (modified)
        {
            lssCout << "Warning: data was modified in the compartment proposal process to get valid starting compartments.\n";
            if (config -> dataModel == 0)
            {
               lssCout << "Error: non-overdispersion data model can not accommodate data modification. Add overdispersion to data model\n"; 
               throw(-1);
            }
        }
    }
