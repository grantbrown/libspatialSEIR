...
accessible data set to test epidmic models using libspatialSEIR.  
...
* fixed immunity loss SEIRS models
...
The first step is to read in and process the data. For this tutorial, we're going to consider just states which are
part of Health and Human Services region 5. These are:
...
fluDat = fluDat[dat.keep,]
region5 = fluDat$HHS.Region.5..IL..IN..MI..MN..OH..WI.
```
...
years = as.numeric(format(as.Date(fluDat$Date), "%Y"))
yearIdx = which(!duplicated(years))
yearIdx = yearIdx[2:length(yearIdx)] # make sure evenly spaced
...
daysSince2000 = as.numeric(as.Date(fluDat$Date) - as.Date("2000-01-01"))
trigBasis = daysSince2000/365*2*pi
Z = cbind(daysSince2000/365,            # linear term for time
          sin(trigBasis),               # trig basis function 1
          cos(trigBasis),               # trig basis function 2
          sin(trigBasis)*cos(trigBasis) # trig basis function 3
  )
```
...
---------------------------
The same basis used for the infection process may serve well for the reintroduction of recovered individuals to the susceptible category. In addition, we may wish to evaluate to what degree individuals were more greatly susceptible to the H1N1 pandemic in 2009. We therefore introduce a time varying H1N1 term, which is zero before April 2009, jumps to one on April first, and then decays. 


```{r, tidy=FALSE, echo=TRUE}
h1n1_term = (as.Date(fluDat$Date) >= as.Date("2009-04-01"))
...
N = matrix(TotalPop, nrow = nrow(fluDat), ncol = 1)
...
# between compartments. Setting it to a very small value eliminates
# the constraint, but it helps with identifiability. 
...
# Set the sampling mode to 2, block sampling, for speed. 

SEIRmodel$samplingMode = 2
...
These look reasonable, so let's launch the main simulation. As currently implemented, the MCMC samplers encounter distressingly high autocorrelation. This is an active target of optimization, but in the meantime we need to be prepared to run inconveniently many samples. 
...
for (i in 1:10)
{
  ...
  SEIRmodel$updateSamplingParameters(0.2, 
                                     0.05, 
                                     0.1)
  cat(paste(i, "of 100,", 
            round((tm/(i)*(10-i)/60), 2), 
            " minutes remaining.\n"))
}
...
```{r tidy = FALSE}
  ...
  par(xpd=TRUE)
  legend(x=-130,y=10,legend=c("Susceptible", "Exposed", "Infectious", "Removed"),
...
Let's examine the parameters chains. 
