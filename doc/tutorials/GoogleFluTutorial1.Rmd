...
# Prior parameters. 
priorAlpha_gammaEI = 23000;
priorBeta_gammaEI = 10000;
priorAlpha_gammaIR = 23000;
priorBeta_gammaIR = 10000;
...
One potential approach to reducing autocorrelation is to sample correlated paramters together in a block. Let's tell libspatialSEIR to do this for the 
R to S transition parameters and transition matrix. 

```{r tidy=FALSE}
SEIRmodel$hybridReinfectionSampling=1
```
...
for (i in 1:100)
{
  tm = tm + system.time(SEIRmodel$simulate(1000))[3]
  SEIRmodel$updateSamplingParameters(0.1, 
                                     0.025, 
                                     0.05)
  cat(paste(i, "of 100,", 
...
{
  chainIdx = floor(length(chain)/2):length(chain)
...
```{r tidy=FALSE}
  ...
  Z.pred = cbind(daysSince2000.pred/365,            # linear term for time
  ...
  predict.i = function(i)
  {
    ...
    return(predictEpidemic(beta, 
                           ...
                           dataRow$E_0_207,
                           dataRow$E_0_207,
                           dataRow$R_0_207)) # These values are made available by setTrace, 
                                             # and are named like: Comp_loc#_time#
  }
  
  preds = lapply((nrow(chainDat) - floor(nrow(chainDat)/4)):
