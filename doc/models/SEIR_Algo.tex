...
\item{Full Conditional for $\{\pi_{j}^{(RS)}\}$}
    ...
    \begin{multline}
        ...
        log(p(\{\pi_{j}^{(RS)}\}|.))\propto\sum_{j=1}^T\Big\{log(\pi_j^{(RS)})\Big[\frac{1}{2} + \sum_{i=1}^n\{S^*_{ij}\}\Big] \\
            \ \ \ \ + 
            log(1-\pi_j^{(RS)})\Big[\frac{1}{2} + \sum_{i=1}^n\{f_R(R^*, S^*, A_0)_{ij} - S^*_{ij}\}\Big]\Big\}\\ 
            \Rightarrow \pi_j^{(RS)} \sim\ (indep)\ beta(\frac{3}{2} + \sum_{i=1}^n\{S^*_{ij}\},
            \frac{3}{2} + \sum_{i=1}^n\{f_R(R^*, S^*, A_0)_{ij}\} - \sum_{i=1}^n\{S^*_{ij}\})
    \end{multline}
\end{center}
...
\item{Full Conditional for $\pi^{(EI)}$} 
    \begin{center}
        \begin{multline}
            ...
            \\+ log(1-\pi^{(EI)})\Big[\frac{1}{2} + \sum_{i=1}^n\sum_{j=1}^T\{(f_E(E^*, I^*, A_0)_{ij} - I^*_{ij})\}\Big]\\
            \Rightarrow \pi^{(EI)} \sim beta(\frac{3}{2} + \sum_{i=1}^n\{I^*_{ij}\}, 
            \frac{3}{2} + \sum_{i=1}^n\{f_E(E^*, I^*, A_0)_{ij}\} - \sum_{i=1}^n\{I^*_{ij}\}) 
        \end{multline}
    \end{center}
\item{Full Conditional for $\pi^{(IR)}$} 
    \begin{center}
        \begin{multline}
            ...
            log(1-\pi^{(IR)})\Big[\frac{1}{2} + \sum_{i=1}^n\sum_{j=1}^T\{ f_I(I^*, R^*, A_0)_{ij} - R^*_{ij}\}\Big]\\
            \Rightarrow \pi^{(IR)} \sim beta(\frac{3}{2} + \sum_{i=1}^n\{R^*_{ij}\}, 
            \frac{3}{2} + \sum_{i=1}^n\{f_I(I^*, R^*, A_0)_{ij}\} - \sum_{i=1}^n\{R^*_{ij}\})
        \end{multline}
    \end{center}
